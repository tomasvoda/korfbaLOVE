...
import KalendarAkci from './pages/KalendarAkci'
import DetailAkce from './pages/DetailAkce'
import NovaAkce from './pages/NovaAkce'
        ...
        >
            {isExpanded ? (
                <div className="flex items-center gap-3 sm:gap-5">
                    ...
                    {/* KALENDÁŘ AKCÍ */}
                    <Link to="/akce" className={`${baseIconStyle} ${location.pathname.startsWith('/akce') ? activeStyle : inactiveStyle}`}>
                        <Calendar className="w-6 h-6" />
                        {location.pathname.startsWith('/akce') && <ActiveDot />}
                    </Link>
                    ...
                    {location.pathname === '/' ? <Home className="w-6 h-6 text-white" /> :
                        location.pathname === '/admin' ? <Crown className="w-6 h-6 text-white" /> :
                            location.pathname === '/login' ? <Shield className="w-6 h-6 text-white" /> :
                                location.pathname.startsWith('/akce') ? <Calendar className="w-6 h-6 text-white" /> :
