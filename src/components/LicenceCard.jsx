    ...
    return (
        <div onClick={onClick} className="relative group glass-panel rounded-2xl overflow-hidden cursor-pointer hover:bg-white/5 transition-all active:scale-[0.98] border border-white/5 hover:border-white/10">
            ...
            <div className="pt-4 pb-3 pl-5 pr-8 flex flex-col gap-3">
                {/* HLAVIČKA: Úroveň + Role + Stav */}
                <div className="flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        <span className={`text-3xl font-black text-${barva}-500 leading-none`}>{licence.uroven}</span>
                        <div className="h-6 w-px bg-white/10"></div>
                        <div className="flex flex-col">
                            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider leading-none mb-0.5">Role</span>
                            <span className="text-sm font-bold text-white leading-none">{licence.typ_role}</span>
                        </div>
                    </div>
                    {jePlatna ? (
                        <span className="flex items-center gap-1 text-[10px] font-bold text-green-400 bg-green-500/10 px-2 py-1 rounded-lg border border-green-500/20"><CheckCircle className="w-3 h-3"/> AKTIVNÍ</span>
                    ) : (
                        <span className="flex items-center gap-1 text-[10px] font-bold text-red-400 bg-red-500/10 px-2 py-1 rounded-lg border border-red-500/20"><AlertTriangle className="w-3 h-3"/> NEAKTIVNÍ</span>
                    )}
                </div>
                ...
                <div className="flex justify-between items-center w-full relative py-1">
                    <div className="flex flex-col">
                        ...
                        <span className="text-xs text-slate-400 font-medium">{dateFmt(licence.datum_ziskani)}</span>
                    </div>
                    <div className="h-px flex-1 bg-gradient-to-r from-transparent via-white/5 to-transparent mx-4"></div>
                    <div className="flex flex-col items-end">
                        <span className="text-[9px] font-bold text-slate-600 uppercase tracking-wider mb-0.5">Platnost do</span>
                        <span className={`text-xs font-medium ${jePlatna ? "text-slate-300" : "text-red-400 font-bold"}`}>{dateFmt(licence.platnost_do)}</span>
                    </div>
                </div>
                ...
                <div className="mt-1 pt-3 border-t border-white/5 flex flex-col gap-3">
                        ...
                        <div>
                            <div className="flex justify-between items-end mb-2 text-xs">
                                <div className="flex items-center gap-1.5 text-blue-300">
                                    <TrendingUp className="w-3.5 h-3.5"/><span className="font-bold">{stats.current}</span><span className="text-[10px] text-slate-500 font-normal">aktuálně</span>
                                </div>
                    ...
                    {/* AKČNÍ LIŠTA (Dny + Tlačítka) */}
                    <div className="flex justify-between items-center pt-1 min-h-[28px]">
                        <div className={`flex items-center gap-1.5 text-[11px] font-medium ${daysLeftColor}`}>
                            {!jePlatna ? <AlertCircle className="w-3 h-3"/> : <Clock className="w-3 h-3 opacity-50"/>}
                            {daysLeftText}
                        </div>
                        ...
                        <div className="flex gap-2 relative z-20" onClick={(e) => e.stopPropagation()}>
                            ...
                            {jeAdmin && (
                                <>
                                    ...
                                    <button onClick={() => onDelete(licence.id)} className="flex items-center gap-1.5 text-[10px] font-bold bg-red-500/10 hover:bg-red-500 hover:text-white text-red-400 px-2 py-1.5 rounded-lg transition-all border border-red-500/10" title="Smazat licenci">
                            ...
                            {!jeAdmin && canEdit && (
                                licence.zadost_o_prodlouzeni ? (
                                    <div className="flex items-center gap-2 text-[10px] font-bold text-yellow-500 bg-yellow-500/10 px-2 py-1.5 rounded-lg border border-yellow-500/20 cursor-default">
                                        <Clock className="w-3 h-3"/> Žádost odeslána
                                    </div>
                                ) : (
                                    <button onClick={() => onRequest(licence.id)} className="flex items-center gap-2 text-[10px] font-bold bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-lg transition-colors shadow-lg shadow-blue-900/20 active:scale-95">
                                        <Send className="w-3 h-3"/> Požádat o prodloužení
                                    </button>
                                )
                            )}
                        </div>
